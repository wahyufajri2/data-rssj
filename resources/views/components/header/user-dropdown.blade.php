<div class="relative" x-data="{
    ...
    <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown()" type="button">
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11">
            <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=random"
                alt="Inisial Nama Pengguna" />
        </span>

        <span class="block mr-1 font-medium text-theme-sm">
            @php
                // Mengambil kata pertama dari nama lengkap pengguna
                $namaDepan = strtok(trim(Auth::user()->name), ' ');
            @endphp
            {{ $namaDepan }}
        </span>
    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <div class="px-3 py-2">
            <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-200">
                {{ Auth::user()->name }}
            </span>
            ...
            @php
                ...
                $menuItems = [
                    [
                        ...
                        'path' => $profileRoute, // Memanggil variabel yang sudah dikondisikan di atas
                    ],
                    // [
                    //     'text' => 'Dukungan',
                    //     'icon' => '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
