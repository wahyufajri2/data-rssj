...
<div class="relative" x-data="{
    ...
    <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown()" type="button">
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11">
            <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->fullname) }}&background=random"
                alt="Inisial Nama Pengguna" />
        </span>

        <span class="block mr-1 font-medium text-theme-sm">
            @php
                $namaSumber = in_array(Auth::user()->peran_id, [Role::ADMIN->value, Role::VERIFIKATOR->value])
                    ? Auth::user()->fullname
                    : session('fullname') ?? Auth::user()->fullname;

                $namaDepan = strtok(trim($namaSumber), ' ');
            @endphp
    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <div>
            <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">
                @if (in_array(Auth::user()->peran_id, [1, 3]))
                    {{ Auth::user()->fullname }}
                @else
                    {{ session('fullname') ?? Auth::user()->fullname }}
                @endif
