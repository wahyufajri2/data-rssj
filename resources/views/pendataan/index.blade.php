            ...
            <table class="w-full whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white font-medium">{{ $item->nama_kk }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $item->ranting->nama_ranting ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm">
                            @if($item->status_kesehatan === 'sehat')
                                <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">Sehat</span>
                            @elseif($item->status_kesehatan === 'resiko')
                                <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400">Resiko</span>
                            @else
                                <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">Gangguan Jiwa</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $item->created_at->format('d M Y') }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada data pendataan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
