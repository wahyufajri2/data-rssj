<div class="flex justify-center gap-2">
    <button type="button" 
        x-data="{ 
            ...
            nama: '{{ addslashes($row->nama_ranting) }}', 
            sk: '{{ addslashes($row->no_sk) }}' 
        }" 
        @click="$dispatch('open-edit-modal', { id: rantingId, cabangId: cabangId, nama: nama, sk: sk })"
    ...
    <form action="{{ route('superadmin.ranting.destroy', $row->id) }}" method="POST" class="inline-block" x-data @submit.prevent="Swal.fire({
        title: 'Hapus Ranting?',
        text: 'Apakah Anda yakin ingin menghapus ranting {{ addslashes($row->nama_ranting) }}?',
        icon: 'warning',
