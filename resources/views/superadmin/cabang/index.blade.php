...
<div class="p-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Manajemen Data Cabang</h2>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <button type="button" x-data @click="$dispatch('open-modal', 'add-cabang')" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors shadow-sm flex items-center justify-center gap-2 w-full sm:w-auto h-[38px]">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden p-4">
        <table id="cabangTable" class="w-full whitespace-nowrap table-auto text-gray-900 dark:text-gray-100">
    ...
    <div x-data="{ open: false }" 
         x-show="open" 
         @open-modal.window="if ($event.detail === 'add-cabang') open = true"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="open" 
                ...
                <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white mb-4">Tambah Data Cabang</h3>
                <form action="{{ route('superadmin.cabang.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="daerah_id" required class="w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white py-2 px-3">
                                <option value="" disabled selected>Pilih Daerah</option>
                                @foreach($daerahs as $daerah)
                                    <option value="{{ $daerah->id }}">{{ $daerah->nama_kabupaten_kota }} - {{ $daerah->nama_wilayah }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama Kecamatan</label>
                            <input type="text" name="nama_kecamatan" required class="w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white py-2 px-3">
                        </div>
                        <div>
    ...
    <!-- Modal Edit Cabang -->
    <div x-data="{ open: false, id: '', daerahId: '', nama: '', sk: '' }" 
         x-show="open" 
         @open-edit-modal.window="open = true; id = $event.detail.id; daerahId = $event.detail.daerahId; nama = $event.detail.nama; sk = $event.detail.sk;"
         @keydown.escape.window="open = false"
         class="fixed inset-0 z-50 overflow-y-auto" 
         style="display: none;">
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="open" 
                ...
                <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white mb-4">Edit Data Cabang</h3>
                <form :action="`/superadmin/cabang/${id}`" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Daerah</label>
                            <select name="daerah_id" x-model="daerahId" required class="w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white py-2 px-3">
                                @foreach($daerahs as $daerah)
                                    <option value="{{ $daerah->id }}">{{ $daerah->nama_kabupaten_kota }} - {{ $daerah->nama_wilayah }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama Kecamatan</label>
                            <input type="text" name="nama_kecamatan" x-model="nama" required class="w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white py-2 px-3">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">No SK</label>
...
<script>
    $(document).ready(function() {
        var table = $('#cabangTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('superadmin.cabang.index') }}",
                data: function (d) {
            ...
            columns: [
                ...
                { data: 'daerah_nama', name: 'daerah.nama_kabupaten_kota' },
